// Note: Chris - Some parameters values and names are assumed. Please update with real names on check-ins
...
begin
  ...
  pipe_rate_reg <= {pipe_rate_reg[5:0], tx_rate_comm[1:0]};
...
cpm4_v1_0_7_versal_smsw_sys_clk_gen_ps #(.offset(7500),.halfcycle(500))  clk_gen_1000 (.sys_clk(clk_1000));
cpm4_v1_0_7_versal_smsw_sys_clk_gen_ps #(.offset(7000),.halfcycle(1000)) clk_gen_500  (.sys_clk(clk_500));
cpm4_v1_0_7_versal_smsw_sys_clk_gen_ps #(.offset(6000),.halfcycle(2000)) clk_gen_250  (.sys_clk(clk_250));
cpm4_v1_0_7_versal_smsw_sys_clk_gen_ps #(.offset(4000),.halfcycle(4000)) clk_gen_125  (.sys_clk(clk_125));
cpm4_v1_0_7_versal_smsw_sys_clk_gen_ps #(.offset(0000),.halfcycle(8000)) clk_gen_62_5 (.sys_clk(clk_62_5));

// Versal should not use these generated clocks because BUFG_GTs are internal to the CPM SecureIPs.
// Versal should use BUFGGT_DIVs
assign mcap_clk = (
  C_CPM_PCIE0_USER_CLK_FREQ == "500_MHz" ||
  C_CPM_PCIE0_USER_CLK_FREQ == "250_MHz" ||
  C_CPM_PCIE0_USER_CLK_FREQ == "125_MHz" ) ? clk_125 : user_clk;

assign pipe_clk_fr =
   tx_rate_comm[1:0] == 2'b0 ? clk_125 :
  (tx_rate_comm[1:0] == 2'b1 ? clk_250 : clk_500); // G1 = 125; G2 = 250; G3/G4 = 500; G5 = 1000;

// pipe_clk assigned at the wires file
assign core_clk =
  C_CPM_PCIE0_CORE_CLK_FREQ == "1000_MHz" ? clk_1000 :
  C_CPM_PCIE0_CORE_CLK_FREQ == "500_MHz"  ? clk_500  : clk_250;

assign user_clk =
  C_CPM_PCIE0_USER_CLK_FREQ == "500_MHz" ? clk_500 :
  C_CPM_PCIE0_USER_CLK_FREQ == "250_MHz" ? clk_250 :
  C_CPM_PCIE0_USER_CLK_FREQ == "125_MHz" ? clk_125 :  clk_62_5;

// BUFG_GT DIVs
//generate
if (C_CPM_PCIE0_CORE_CLK_FREQ == "1000_MHz") // This frequency is special for Gen4x16 only
begin
  assign txout_clk = clk_1000;
  assign pipeclk_div =
    tx_rate_comm[1:0] == 2'b0  ? 'd7 :
    tx_rate_comm[1:0] == 2'b1 ? 'd3 : 'd1; // G1 = 125; G2 = 250; G3/G4 = 500;

  assign coreclk_div = 0;    // Coreclk = TXoutclk
  assign userclk_div = 'd1;  // 500MHz is the only option for this config for CPM4
  assign userclk2_div = 'd0; // 1000MHz is the only option for this config for CPM4
end
if (C_CPM_PCIE0_CORE_CLK_FREQ == "500_MHz") // This frequency doesn't support gen4x16
begin
  assign txout_clk   = clk_500;
  assign pipeclk_div = (tx_rate_comm[1:0] == 2'b0)? 'd3 :
                       (tx_rate_comm[1:0] == 2'b1)? 'd1 :
                                                    'd0; // G1 = 125; G2 = 250; G3/G4 = 500;
  assign coreclk_div = 0; // Coreclk = TXoutclk
  assign userclk_div = C_CPM_PCIE0_USER_CLK_FREQ == "250_MHz" ? 'd1 :
...
if (C_CPM_PCIE0_CORE_CLK_FREQ == "250_MHz") // This frequency doesn't support gen3/4
begin
